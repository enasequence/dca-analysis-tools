...
def fetch_and_filter_seq(connection, output):
   # This Part is for querying ENAPRO
    ...
    header = "\t".join(['Accession', 'First_public', 'Last_public', 'status_id', 'presentation_status'])
    f.write(str(header) + "\n")
    accession_list_seq = []
    for accession in output:
        ...
        for row in c:
            server = "https://www.ebi.ac.uk/ena/browser/api/summary"
            ext = "/{}".format(accession)
            ...
            f.write(str(accession) + "\t" + str(row[0]) + "\t" + str(row[1]) + "\t" + str(row[2]) + "\t" + str(presentation_status) +"\n" )
...
def fetch_and_filter_reads(connection, output):
    ...
    header = "\t".join(['Accession', 'Status_id' 'First_public', 'Last_updated', 'presentation_status'])
    f.write(str(header) + "\n")
...
def covid_advanced_search_difference (ena_dataset, covid_reads_portal_df):
    ...
    return [covid_portal_output, covid_duplicate_list]
...
def presentation ():
    server = "https://www.ebi.ac.uk/ena/browser/api/summary"
    ext = "/A00001".format(database, tax_fetch[0], acc, date)
    command = requests.get(server + ext, headers={"Content-Type": "application/json"})
...
if database == 'reads':
    ...
    summary_log = open(f'{args.file}/summary_log_{database}.txt', 'w')
    ...
    summary_log.write(f'Number of Reads in NCBI (SRA database) is: {len(sra_df)}\n')
    ...
    summary_log.write(f'Number of Reads in ENA Advanced Search is: {len(ena_read_df)}\n')
    ...
    summary_log.write(f'Number of Reads in EBI Search is: {len(ebi_read_df)}\n')
    ...
    summary_log.write(f'Number of Reads in COVID-19 data portal is: {len(covid_reads_portal_df)}\n')

    #Obtain the difference between the data in NCBI and ENA
    output = reads_dataset_difference()
    summary_log.write(f'Number of data found in NCBI ( SRA database) but missing in ENA advanced search is: {len(output)}\n')

    #Obtain the reads difference between Advanced search and COVID-19 Portal, and duplicates if present
    portal_reads= covid_advanced_search_difference(ena_read_df, covid_reads_portal_df)
    summary_log.write(
        f'Number of {database} found in ENA advanced search but missing in COVID-19 data portal is: {len(portal_reads[0])}\n')
    summary_log.write(
        f'Number of {database} found duplicated in COVID-19 data portal is: {len(portal_reads[1])}\n')

    # Obtain the reads difference between Advanced search and EBI search
    adv_vs_ebi_reads = advanced_search_ebi_search_difference(ebi_read_df, ena_read_df)
    summary_log.write(
        f'Number of {database} found in EBI search but missing in ENA advanced search is: {len(adv_vs_ebi_reads[0])}\n')
    summary_log.write(
        f'Number of {database} found in ENA advanced search but missing in EBI search is: {len(adv_vs_ebi_reads[1])}\n')
    summary_log.close()
    ...
    summary_log = open(f'{args.file}/summary_log_{database}.txt', 'w')
    ncbivirus_set = set(open(f"{args.file}/NCBI.nucleotide.log.txt").read().split())
    print('Number of Sequences in NCBI (Nucleotide database) is: ', len(ncbivirus_set))
    summary_log.write(f'Number of Sequences in NCBI (Nucleotide database) is: {len(ncbivirus_set)}')
    ...
    summary_log.write(f'Number of Sequences in ENA Advanced Search is:  {len(ena_seq_df)}')
    ...
    summary_log.write(f'Number of Sequences in EBI Search is:  {len(ebi_seq_df)}')
    ...
    summary_log.write(f'Number of Sequences in COVID-19 data portal is:  {len(covid_reads_portal_df)}')

    #Obtain the difference between the data in NCBI and ENA
    output = sequence_dataset_difference()
    summary_log.write(
        f'Number of data found in NCBI ( Nucleotide database) but missing in ENA advanced search is:  {len(output)}\n')

    #Obtain the sequence difference between Advanced search and COVID-19 Portal, and duplicates if present
    portal_seq = covid_advanced_search_difference(ena_seq_df,covid_reads_portal_df)
    summary_log.write(
        f'Number of {database} found in ENA advanced search but missing in COVID-19 data portal is: {len(portal_seq[0])}\n')
    summary_log.write(
        f'Number of {database} found duplicated in COVID-19 data portal is: {len(portal_seq[1])}\n')

    # Obtain the reads difference between Advanced search and EBI search
    adv_vs_ebi_seq = advanced_search_ebi_search_difference(ebi_seq_df, ena_seq_df)
    summary_log.write(
        f'Number of {database} found in EBI search but missing in ENA advanced search is: {len(adv_vs_ebi_seq[0])}\n')
    summary_log.write(
        f'Number of {database} found in ENA advanced search but missing in EBI search is: {len(adv_vs_ebi_seq[1])}\n')
    summary_log.close()
    ...
    error_log = open(f'{args.file}/error_log.txt', 'w')
    sys.stderr.write(f'The dataset type "{database}" does not exist, please check your spelling and try again')
    error_log.write(f'The dataset type "{database}" does not exist, please check your spelling and try again')
    error_log.close()
