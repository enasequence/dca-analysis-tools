...
parser.add_argument('-o', '--output', help='Output path', type=str, required=False)
parser.add_argument('-of', '--folder', help='Output folder name', type=str, required=False)
args = parser.parse_args()

# generate and create the output directory
def create_outdir():
    if args.output == None:
        output = input("specify the path of the output directory: ").lower()
    else:
        output = args.output
    if args.folder != None:
        outdir = f"{args.output}/{args.folder}"
    else:
        now = datetime.datetime.now()
        now_str = now.strftime("%d%m%y")
        outdir = output   #/databases_logs_{now_str}
...
def advanced_search_data_fetching(database, outdir):
    acc = 'accession'
    date ='last_updated'
...
def covid19portal_data_fetching(database):
    ...
    while start <= now:
        ...
        while page >= 0:
            ...
            ext = "/{}?query=TAXON:2697049 AND {}:{}-{}&page={}&size=1000&format=json".format(database,dateformat,year,month, page)
            command = requests.get(server + ext, headers={"Content-Type": "application/json"})
            status = command.status_code
...
#Creating script to fetch data from NCBIvirus ( NCBIvirus API is going to be deprecated, which it might not work in the future and it will be replaced by nucleotide tool)
...
if repository in ['ena', 'ncbi','covid19dataportal', 'ebisearch', 'all']:
    ...
    if repository == 'ncbi':
        ...
        if database in ['sequences', 'reads']:
            ...
            if repository == 'ena':
                advanced_search_data_fetching(database, outdir)
