...
parser = argparse.ArgumentParser(prog='create_multifasta.py', formatter_class=argparse.RawDescriptionHelpFormatter,
        ...
        |                                                              |
        |      Tool to create multifasta from the output of fetch script |
        + =========================================================== +  """)
...
def read_fasta():
    ...
    if not os.path.exists(outdir):
        os.mkdir(outdir)
    total_files = sum([len(files) for r, d, files in os.walk(args.file)])
    print(total_files)
    number_fasta_seq = math.trunc(total_files/int(args.number))
    print(f"Found {total_files} fasta files in {args.file} - writing {number_fasta_seq} sequences to each multifasta file, the rest -if exist- will be merged with the last file **************\n")
    index = 0
    ext = 1
    fasta_list = []
    for i in listdir(args.file):
        if i == "end.txt":
            continue
        for f in listdir(f'{args.file}/{i}'):
            if fnmatch.fnmatch(f, '*.fasta.gz'):
                ...
                with gzip.open(f'{args.file}/{i}/{f}','rb') as f_in:
                    content = f_in.read()
                    fp.write(content)
                fp.close()
